...
function unwrapVolatile(value) {
  if (!objectLike(value) || !(VOLATILE_WRITE in value) || typeof value.get !== 'function') return value
  return value.get()
}

function clone(value) {
  value = unwrapVolatile(value)
  return value === undefined ? undefined : structuredClone(value)
}

function mergeResolved(base, overlay) {
  base = unwrapVolatile(base)
  overlay = unwrapVolatile(overlay)
...
function createConfigEditorSettingsFacade(editor, entryConfig, namespace) {
  ...
  const scope = {
    get() {
      return state()?.descriptor.value ?? clone(entryConfig)
    },
    watch(listener) {
