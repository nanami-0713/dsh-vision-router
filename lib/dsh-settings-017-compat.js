...
function createConfigEditorSettingsFacade(ctx, entryConfig, namespace) {
  ...
  function state() {
    ...
    const rawFingerprint = fingerprint({ current, inherited })
    if (seenFingerprint === undefined) seenFingerprint = rawFingerprint
    else if (rawFingerprint !== seenFingerprint) {
      seenFingerprint = rawFingerprint
      revision += 1
    }
  ...
  const facade = {
    ...
    async mutate(namespaceName, operations, expectedRevision) {
      ...
      if (!Number.isInteger(expectedRevision) || expectedRevision < 0) {
        throw admissionError('expectedRevision must be a non-negative integer')
      }
...
function contextWithSettingsFacade(ctx, facade) {
  const wrap = (target) => new Proxy(target, {
    get(object, property) {
      if (property === 'settings') return facade
      if (property === 'get') {
        const get = Reflect.get(object, property, object)
        if (typeof get !== 'function') return get
        return (name, ...rest) => name === 'settings' ? facade : get.call(object, name, ...rest)
      }
      if (property === 'inject') {
...
export function installDsh017SettingsCompatibility(ctx, entryConfig = {}, options = {}) {
  ...
  const facade = createConfigEditorSettingsFacade(ctx, entryConfig, namespace)
  return contextWithSettingsFacade(ctx, facade)
}
...
export function installDsh017LocalSettingsTransport(ctx) {
  ...
  ctx.inject(['settings', 'webServer'], (webCtx) => {
    if (!webCtx.settings || typeof webCtx.settings.describe !== 'function' || typeof webCtx.settings.mutate !== 'function') return
    webCtx.effect(
