...
/**
 * DSH 0.1.7 services may mount after DVR's apply() starts. Resolve the settings
 * generation when each service access/injection actually occurs, rather than
 * freezing one startup-time guess.
 */
function contextWithLazySettingsCompatibility(ctx, entryConfig, namespace) {
  ...
  function compatibleSettings(target) {
    const settings = serviceOf(target, 'settings')
    if (!settings || typeof settings.register === 'function') return settings
    const editor = serviceOf(target, 'configEditor')
    ...
    if (!facade) {
      facade = createConfigEditorSettingsFacade(target, entryConfig, namespace)
