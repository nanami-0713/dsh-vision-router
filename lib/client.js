...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const zh = {
      ...
      proxyLabel: '视觉专用代理覆盖',
      proxyHint: '留空即沿用 DSH/Host 网络路径。仅需单独覆盖视觉请求时填写，如 http://127.0.0.1:10808 或 socks5://127.0.0.1:10808；兼容旧配置 socks5h://。修改即时生效。',
      ...
      proxyHostsLabel: '视觉代理覆盖域名（每行一个）',
      proxyHostsHint: '仅在上方填写视觉专用代理时生效；这些域名使用插件覆盖，其余请求继续沿用 DSH/Host 网络路径。留空清除覆盖。',
      ...
      hintStealth:
        '只作用于官方 DeepSeek 路由。新版 DSH 中官方 provider 始终归 Host 所有，Vision Router 只使用「+ 自动识图」' +
        '包装，不再要求你禁用 llm-deepseek；旧版 Host 仍保留官方行缺失时的历史接管兼容。opencode 等自定义路由' +
        '默认自动包装，只有要限制范围时才用下方手动包装。',
      stealthOfficialDeadHint:
        '提示：这是旧版 Host，且官方 DeepSeek 行当前被禁用；Vision Router 正使用兼容性 keep-alive 路由。' +
        '如需恢复完全官方的原生行，请在 cordis.patch.yml 中重新启用 llm-deepseek 后重启。',
      stealthOfficialHostOwnedHint:
        '检测到官方 DeepSeek provider 被禁用或不可用，但当前 DSH 已由 Host 负责该路由。Vision Router 不会自行重建它。' +
        '请重新启用 llm-deepseek，再通过「+ 自动识图」包装 /「👁 识图」处理图片任务。',
    ...
    const en = {
      ...
      proxyLabel: 'Vision-only proxy override',
      proxyHint: 'Leave empty to follow the DSH/Host network path. Set only to override vision requests, e.g. http://127.0.0.1:10808 or socks5://127.0.0.1:10808; legacy socks5h:// values are supported. Applies immediately.',
      ...
      proxyHostsLabel: 'Vision proxy override hosts (one per line)',
      proxyHostsHint: 'Used only when the vision-only proxy override above is set. These hosts use the plugin override; all other requests keep the DSH/Host network path. Empty clears the override.',
      ...
      hintStealth:
        'Only affects the official DeepSeek route. On newer DSH Hosts the official provider stays Host-owned, so Vision Router ' +
        'uses the “+ Auto Vision” wrapper and never asks you to disable llm-deepseek. Legacy Hosts retain the historical ' +
        'takeover compatibility when the stock row is absent. Custom routes like opencode are auto-wrapped by default; use ' +
        'the manual wrapper section below only when you want to restrict their scope.',
      stealthOfficialDeadHint:
        'Notice: on this legacy Host the official DeepSeek row is disabled, so Vision Router is using its compatibility ' +
        'keep-alive route. To restore the fully official route, re-enable llm-deepseek in cordis.patch.yml and restart.',
      stealthOfficialHostOwnedHint:
        'The official DeepSeek provider is disabled or unavailable, but this DSH Host owns that provider route. Vision Router ' +
        'will not recreate it. Re-enable llm-deepseek, then use the “+ Auto Vision” wrapper / Vision control for image tasks.',
    ...
    function VisionRouterCard(props) {
      ...
      const stealthNotice = () => {
        ...
        if (state.reason === 'host-owned-official-unavailable') {
          return h('p', { className: 'vr-hint vr-stealth-notice' }, t('stealthOfficialHostOwnedHint'))
        }
        if (state.active !== true || state.reason !== 'official-unavailable') return null
      ...
      ? h('div', { className: 'vr-advanced' },
          ...
          !remoteMode ? h('div', { className: 'vr-group' },
            ...
            h('p', { className: 'vr-hint' }, t('networkAuthorityHint')),
