...
function isCompositeWrapperModel(model) {
...
async function mainWrapperModel(ctx, fallbackWrapperRoute, model, signal) {
  ...
  // Composite wrapper ids are owned by the Core wrapper. Delegating them before
  // consulting the official DeepSeek catalog keeps a transient directory outage
  // from disabling unrelated configured/local vision backends. Core validates
  // the exact routingPairs() id before resolving it; an unknown "foo/bar" still
  // falls through to Core's strict official-catalog path, so authority is not
  // broadened here.
  if (isCompositeWrapperModel(model)) return undefined
