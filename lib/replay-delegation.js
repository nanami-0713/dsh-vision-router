...
function configuredCompositeWrapperModel(ctx, model) {
  if (!isNonEmptyString(model)) return false
  const separator = model.indexOf('/')
  if (separator <= 0 || separator === model.length - 1) return false
  const provider = model.slice(0, separator)
  const backendModel = model.slice(separator + 1)
  return configuredVisionAdapterModels(effectiveVisionConfig(ctx, {}))
    .get(provider)
    ?.has(backendModel) === true
}
...
async function mainWrapperModel(ctx, fallbackWrapperRoute, model, signal) {
  ...
  // Config-authorized composite rows are owned by the Core wrapper. Delegating
  // them before consulting the official DeepSeek catalog keeps a transient
  // directory outage from disabling unrelated configured vision backends. Core
  // still validates the exact routing pair; this does not broaden authority.
  if (configuredCompositeWrapperModel(ctx, model)) return undefined
