...
function syncAgentToolRestriction(agent, enabled, restrictions, owned) {
  ...
  const deny = restrictableOwnedVisionToolNames(tools, owned)
  ...
  try {
    ctx.on?.('system-prompt/assemble', async (assembly, context, next) => {
      ...
      const projectedOwned =
        isObject(agent) && agent?.ctx?.tools
          ? new Set(restrictableOwnedVisionToolNames(agent.ctx.tools, ownedVisionTools))
          : ownedVisionTools
