...
- **大型 Session surface repair 改为事件流驱动（#524、#532）**：在受支持 DSH Host 上，生产 repair 订阅 post-commit `session/event`，只缓存确需替换的 tool-result / guard-stop 事件，并在 pre-step 与当前 Host surface 求交后处理；不再从 live Session 调用 `sessionQuery.readEvent()`、读取 `observeSession.events`、`eventAt()` 或 `snapshotEvents()`。这同时消除 2.2.0 的重复全量 clone，以及后续现场复现中的 `Unable to deserialize cloned data` / Windows Node 24 原生崩溃路径；没有事件流边界的 isolated/partial caller 仍保留兼容 reader。
- **Large-session surface repair is now event-feed driven (#524, #532)**: on supported DSH Hosts, production repair subscribes to post-commit `session/event`, retains only tool-result / guard-stop events that may need replacement, and consumes them only while their seq is still on the current Host-owned surface. The production path no longer calls `sessionQuery.readEvent()`, materializes `observeSession.events`, or uses `eventAt()` / `snapshotEvents()`. This removes both the 2.2.0 full-clone amplification and the later `Unable to deserialize cloned data` / Windows Node 24 native-crash path; isolated/partial callers without the event-feed boundary keep the compatibility reader.
...
- **视觉代理 redirect 逐跳边界根修**：显式 `proxy` 的请求不再把首跳命中的 ProxyAgent 固定到整条 Fetch redirect 链。Router-owned 与 Host-owned scoped 两条路径现在共用 per-hop dispatcher selector：首跳只有命中 `proxyHosts` 才进入 DVR 覆盖；进入后每个 redirect hop 都按目标 origin 重新校验，仍在名单内才走 DVR ProxyAgent，离开名单立即回到该请求开始时快照的 Host/caller dispatcher。Fetch/Undici 继续独占 301/302/303/307/308、body replay、header stripping、redirect limit 与 AbortSignal 语义；首跳未命中的请求仍完整沿用 Host，且保持 #149 的零 DVR Undici 导入。两条显式代理路径同时改用共享 lease/retire dispatcher pool，避免 selector 隐藏底层 Agent 后产生生命周期泄漏。
- **Per-hop redirect boundary for vision proxy overrides**: an explicit `proxy` no longer pins the first-hop ProxyAgent across the entire Fetch redirect chain. Router-owned and scoped Host-owned paths now share a per-hop dispatcher selector: DVR participates only when the initial URL matches `proxyHosts`; once admitted, every redirect hop re-checks its target origin, uses the DVR ProxyAgent only while still listed, and returns to the request-start Host/caller dispatcher as soon as it leaves the list. Fetch/Undici remains the sole owner of 301/302/303/307/308, body replay, header stripping, redirect limits, and AbortSignal semantics. An initially non-matching request remains entirely Host-owned and preserves #149's zero-DVR-Undici path. Both explicit-proxy paths also share one lease/retire dispatcher pool so the selector cannot hide ProxyAgent lifecycle from cleanup.
...
- **Host-owned 视觉代理作用域隔离（H2）**：显式 Vision Router 代理不再因“配置里存在 Host-owned 视觉模型”就接管同域的进程级 `fetch`。ProxyAgent 与兼容 URL 投影已从 Core 移到单一兼容边界；该边界默认完整透传现有 DSH/Host fetch 链，只在 AsyncLocalStorage 证明当前正在执行已配置的 Host-owned 视觉 adapter（视觉链、`vision_describe`、Benchmark/Exact Check）时注入 DVR dispatcher。并发的同域 Host 请求不会被误代理，热修改 `proxy` 继续即时生效。仅 `routing=true` 且显式清空 `chainRoute` 的旧整轮直连模式保留无 scope 兜底，等待 H3 产品策略处理。
- **Scoped Host-owned vision proxy isolation (H2)**: an explicit Vision Router proxy no longer claims process-wide same-origin `fetch` traffic merely because a Host-owned vision model exists in configuration. ProxyAgent ownership and legacy URL projection move out of Core into one compatibility boundary that normally delegates the existing DSH/Host fetch chain unchanged; it injects a DVR dispatcher only while AsyncLocalStorage proves an active configured Host-owned vision adapter call (vision chain, `vision_describe`, Benchmark/Exact Check). Concurrent Host traffic to the same origin is not proxied, and live `proxy` edits still apply immediately. Only the legacy direct whole-turn mode with `routing=true` and an explicitly blank `chainRoute` retains an unscoped fallback pending H3 product policy.
...
- **OpenCode Go 会话亲和传播根修（#410）**：视觉工具发起的 Harness 子调用现在透传 DSH 真实 `sessionId`；只有绕过 Harness 的 OpenCode Go direct bridge 才动态发送 `x-opencode-session`，并保留 DSH 原始会话 ID，不做 UUID 形态重写。缺少稳定会话身份时 direct Go 请求在本地 fail-closed，不发送全局占位 ID。`vision-chain` 正常 Harness 分支继续沿用原有 `...options` 语义，catalog-correction / `vision-http` / tool direct fallback 补齐此前丢失的上下文；capability benchmark 使用每次 benchmark run 独立且稳定的 synthetic session，避免污染真实对话或跨 run 共用身份。
- **OpenCode Go session-affinity propagation root fix (#410)**: child Harness calls spawned by vision tools now preserve DSH's real `sessionId`; only OpenCode Go direct bridges that bypass Harness emit the dynamic `x-opencode-session` header, using DSH's opaque session id unchanged rather than reshaping UUIDs. Direct Go calls without a stable identity fail closed locally instead of sending a global placeholder. The normal `vision-chain` Harness branch keeps its existing `...options` semantics, while catalog corrections, `vision-http`, and tool-side direct fallbacks now retain the previously dropped context. Capability benchmarks use one isolated stable synthetic session per benchmark run so measurements neither contaminate user conversations nor share one global identity across runs.

- **#410 对抗审查补强**：内部 DSH Session identity 与 HTTP wire affinity 现已彻底分离，超长/控制字符会话不再坍缩为共享 `anon` scope；Host/pi-ai 正常流量在 Vision Router 自有 AsyncLocalStorage 作用域内，仅对真实 `https://opencode.ai/zen/go...` 请求补齐缺失的 `x-opencode-session`，上游已原生携带该头时保持 no-op。Unicode/会被 Headers 改写的亲和值在发网前显式拒绝；capability benchmark 使用独立 `affinityId`，不再伪造 DSH `GenerateOptions.sessionId`。
- **#410 adversarial hardening**: internal DSH Session identity is now separate from HTTP wire affinity, so long/control-character session ids cannot collapse into a shared `anon` turn scope. Normal Host/pi-ai traffic gets a missing `x-opencode-session` only inside a Vision Router-owned AsyncLocalStorage scope and only for the real `https://opencode.ai/zen/go...` endpoint; an upstream-native header wins unchanged. Unicode/header-rewritten affinity ids fail before network, and capability benchmarks use an isolated `affinityId` instead of impersonating a DSH `GenerateOptions.sessionId`.
