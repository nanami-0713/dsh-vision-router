...
test('fuzz workflow keeps a fixed PR corpus and rotating scheduled exploration', () => {
  ...
  assert.match(workflow, /Install fuzz harness dependencies/)
  assert.match(workflow, /pnpm install --frozen-lockfile --ignore-scripts/)
  assert.match(workflow, /Property fuzz deterministic security boundaries/)
  assert.match(workflow, /DVR_PROPERTY_FUZZ_CASES: '1000'/)
  assert.match(workflow, /DVR_PROPERTY_FUZZ_SEED: '1592639710'/)
  assert.match(workflow, /pnpm test:property-fuzz/)
  ...
  assert.match(workflow, /Property fuzz rotating security boundaries/)
  assert.match(workflow, /DVR_PROPERTY_FUZZ_CASES: '3000'/)
  assert.match(workflow, /DVR_FUZZ_CASES: '1500'/)
  assert.match(workflow, /steps\.exploration_seed\.outputs\.seed/)
  assert.match(workflow, /GITHUB_STEP_SUMMARY/)
  assert.match(workflow, /tests\/security-property-fuzz\.test\.js/)
